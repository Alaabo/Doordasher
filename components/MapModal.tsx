...
import { Modal, View, TextInput, TouchableOpacity, Text, StyleSheet, Dimensions, FlatList, I18nManager } from "react-native";
...
const LocationSearchModal: React.FC<LocationSearchModalProps> = ({ visible, onClose, dataHandler, location }) => {
                ...
                const response = await axios.post('https://places.googleapis.com/v1/places:searchText',
                    {
                        textQuery: query,
                        pageSize: 5,
                        // Add language parameter for Arabic search results when in RTL mode
    ...
    const rtlStyles = {
        container: {
            flexDirection: isRTL ? ('row-reverse' as 'row-reverse') : ('row' as 'row'),
        },
        textAlign: {
            textAlign: isRTL ? 'right' : 'left',
        },
        paddingStart: {
            paddingStart: isRTL ? 0 : 16,
            paddingEnd: isRTL ? 16 : 0,
        }
    };
            ...
            <View style={styles.overlay}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        >
                            <Text 
                                className="text-xl font-Poppins-medium text-primary-200"
                                style={{ textAlign: isRTL ? 'right' : 'left' }}
                            >
                                {item.displayName.text}
                            </Text>
                            <Text 
                                className="text-xs font-Poppins-Thin text-primary-300"
                                style={{ textAlign: isRTL ? 'right' : 'left' }}
                            >
